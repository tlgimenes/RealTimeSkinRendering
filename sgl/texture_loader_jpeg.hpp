...
typedef struct libjpeg_error_mgr_t 
{
    struct jpeg_error_mgr pub; /* "public" fields */
    jmp_buf setjmp_buffer;     /* for return to caller */
} libjpeg_error_mgr;

typedef libjpeg_error_mgr* libjpeg_error_ptr;
...
libjpeg_error_exit(j_common_ptr cinfo)
{
     ...
     *  */
    libjpeg_error_mgr* myerr = (libjpeg_error_mgr*) cinfo->err;
...
std::shared_ptr<sgl::host::texture2D> sgl::texture_loader_jpeg::load(const std::string& filename) 
{
     ...
     */
    uint counter = 0;
    jpeg_data.resize(cinfo.output_height * row_stride);
    while (cinfo.output_scanline < cinfo.output_height) {
        ...
        memcpy(jpeg_data.data()+counter, buffer[0], row_stride);
        counter += row_stride;
    }
